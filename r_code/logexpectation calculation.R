#This function is used to calculate the log-factorial-expectation value of the COM-Poisson distribution 
...
#the log-expected value is attained by approximation. 
logexpectation<-function(lambda,nu){
...
slot<-rep(0,n)
for (i in 1:n){
slot[i]<-logexpectation(lambda[i],nu[i])
  }
slot<-as.matrix(slot)
return(slot)
 }
...
nu<-as.numeric(nu)
logexpect<-com.logexpect.compute.z(lambda,nu)/com.compute.vecz(lambda,nu)
return(logexpect)
 }
}
